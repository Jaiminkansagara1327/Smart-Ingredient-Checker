...
const SignupPage = ({ onNavigate, onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (errData?.errors) setError(Object.values(errData.errors).flat().join(' '));
      else setError(err.response?.data?.message || 'Failed to create account. Please try again.');
    } finally { setLoading(false); }
  };
  ...
  return (
    <div className="auth-page">
      ...
      <motion.div className="auth-card" initial={{ opacity: 0, scale: 0.94, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <p className="auth-sub">Start your journey toward nutritional truth.</p>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          <div className="auth-field-wrap">
            ...
            <input type="text" className="auth-input" placeholder="John Doe" value={firstName} onChange={e => setFirstName(e.target.value)} required />
          </div>
          <div className="auth-field-wrap">
          ...
          <div className="auth-field-wrap">
            ...
            <input type="password" className="auth-input" placeholder="Min. 8 characters" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} />
          </div>
