...
function UploadSection({ onAnalyze }) {
        ...
        try {
            ...
            if (response.data.is_valid_ingredient_list === false) {
                const validationMsg = response.data.validation_message ||
                    'The text in this image does not appear to be a food ingredient list.';
                setValidationError({
                    title: 'Not a Valid Ingredient List',
                    message: validationMsg,
                    suggestion: 'Please upload an image of a product\'s ingredient list, not random text or other content.'
                });
                return;
            }
    ...
    const handleUrlAnalyze = async () => {
        ...
        try {
            ...
            if (response.data.is_valid_ingredient_list === false) {
                const validationMsg = response.data.validation_message ||
                    'The content from this URL does not appear to be a food ingredient list.';
                setValidationError({
                    title: 'Not a Valid Ingredient List',
                    message: validationMsg,
                    suggestion: 'Please provide a URL that contains a product\'s ingredient list.'
                });
                return;
            }
